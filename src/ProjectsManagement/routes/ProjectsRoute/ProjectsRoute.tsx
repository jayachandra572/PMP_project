...
class ProjectsRoute extends Component<ProjectsRouteProps> {
   ...
   @computed get getApiStatus(): number {
      const { userDetailsStore, projectsStore } = this.injectedProps
      const { getUserDetailsApiStatus } = userDetailsStore
      const {
         pageNavigation: { getApiStatus }
      } = projectsStore
   ...
   onChangePageNumberReaction = reaction(
      () => this.injectedProps.projectsStore.pageNavigation.currentPage,
      () => {
         this.doNetWorkCall()
      }
   )
   ...
   renderSuccessUI = observer(() => {
      const {
         ...
         getApiStatus,
         getApiError
      } = this.injectedProps.projectsStore.pageNavigation
      return (
         <Projects
