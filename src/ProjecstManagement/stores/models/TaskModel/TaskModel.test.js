...
describe("Task model test cases",()=>{
    beforeEach(() => {
      ...
      taskModel = new TaskModel(Tasks[0],changeTaskStatusAPI,postTaskTransitionValidationAPI)
   })
    ...
    it("should test changeTaskSuccess status",async ()=>{
        const mockSuccessPromiseFn = () => new Promise((resolve)=>{resolve(taskStatusChangeResponse)})
    ...
    it("should test changeTaskFailure status",async ()=>{
        const mockFailurePromiseFn = () => new Promise((_,reject)=>{reject(new Error('Error'))})
    ...
    it("Should test change task status", async ()=>{
        taskModel.taskTrasitionState.apiCallFunction = () => new Promise ((resolve)=>resolve([]))
